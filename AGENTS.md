...
- The private `/list` page is for internal price review, hides normal site header/footer, stays visually light-mode, uses Excel-derived product/photo/retail/artist/Mini Wholesale + MOQ/Bulk Wholesale + MOQ data, browser-stored confirmed edits/version snapshots, supports click-to-preview product images, and is protected by the hardcoded session password `tina`. Keep price and MOQ inputs at 16px or larger, use decimal text entry for prices, and require confirm/cancel before applying edits so mobile browsers do not zoom/crop while editing.
