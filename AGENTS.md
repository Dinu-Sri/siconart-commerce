...
- Storefront accepts any order amount. Shipping is calculated at checkout for listed countries; unlisted countries are asked to continue on WhatsApp.
...
- The private `/list` page is for internal price review, hides normal site header/footer, stays visually light-mode, uses product/photo/market/artist/Mini Wholesale + MOQ/Bulk Wholesale + MOQ data, browser-stored confirmed edits/version snapshots, supports click-to-preview product images, and is protected by the hardcoded session password `tina`. Keep price and MOQ inputs at 16px or larger, use decimal text entry for prices, and require confirm/cancel before applying edits so mobile browsers do not zoom/crop while editing.
