...
- Private price list at `/list` is sourced from the master price sheet data, shows editable product cards with image preview, name, market price, artist price, Mini Wholesale price + MOQ, and Bulk Wholesale price + MOQ. It hides normal header/footer, stores confirmed edits and versions in the browser, asks for confirmation before applying price/MOQ changes, and uses password `tina`.
...
- 2026-07-28: Imported clear Mini Wholesale prices/MOQs from the SiconArt MINI Wholesale catalog PDF and changed the private list retail label to Market.
